...
tryCatch({
  ...
  for (resolution in resolutions) {
    ...
    png_file <- file.path(temp_png_folder, sprintf("Resolution:%.2f_ClusterNumber:%d.png", resolution, cluster_number))
    
    png(png_file, width = 18 * 200, height = 18 * 200, res = 200, 
        bg = "white", type = "cairo")
    print(combined_plot)
    dev.off()
    
    all_png_files[as.character(resolution)] <- png_file
    ...
    rm(sobj, findcelltypes_result, heatmap_plot, umap_plot, table_data, 
       dotplot1_plot, dotplot2_plot, combined_plot)
    gc()
    
    message("Finished resolution: ", resolution, " (PNG saved)")
  }
  ...
  pushViewport(viewport(layout = grid.layout(
    nrow = 2, 
    ncol = 1,
    heights = unit(c(0.15, 0.85), "npc")
  )))
  
  pushViewport(viewport(layout.pos.row = 1))
  grid::grid.text("Cluster Analysis Summary", 
                  x = 0.5, y = 0.7,
                  gp = grid::gpar(fontsize = 36, fontface = "bold"))
  grid::grid.text(paste("Generated:", Sys.time()),
                  x = 0.5, y = 0.2,
  ...
  for (i in 1:3) {
    ...
    if (start_row <= end_row) {
      sub_data <- resolution_summary[start_row:end_row, c("resolution", "cluster_number")]
      colnames(sub_data) <- c("Resolution", "Clusters")
  ...
  pushViewport(viewport(layout = grid.layout(
    nrow = 1,
    ncol = 3,
    widths = unit(c(0.33, 0.34, 0.33), "npc"),
    respect = TRUE
  )))
  
  pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 1))
  if (!inherits(tables[[1]], "null")) {
    grid::grid.draw(tables[[1]])
  }
  popViewport()
  
  pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 2))
  if (!inherits(tables[[2]], "null")) {
    grid::grid.draw(tables[[2]])
  }
  popViewport()
  
  pushViewport(viewport(layout.pos.col = 3))
  if (!inherits(tables[[3]], "null")) {
    grid::grid.draw(tables[[3]])
  }
  popViewport()
  ...
  sorted_resolutions <- sort(as.numeric(names(all_png_files)))
  sorted_png_files <- all_png_files[as.character(sorted_resolutions)]
  
  
  plots_per_page <- 4
  total_pages <- ceiling(length(sorted_png_files) / plots_per_page)
  
  for (page in 1:total_pages) {
    
    start_idx <- (page - 1) * plots_per_page + 1
    end_idx <- min(page * plots_per_page, length(sorted_png_files))
    ...
    for (i in seq_along(start_idx:end_idx)) {
      idx <- start_idx + i - 1
      if (idx <= length(sorted_png_files)) {
        png_file <- sorted_png_files[idx]
  ...
  write.xlsx(resolution_summary[, c("resolution", "cluster_number")], file = summary_file)
...
results <- future.apply::future_lapply(resolutions, function(resolution) {
  dyn.load("/projectnb/wax-es/00_shinyapp/Clustering/conda_env/lib/libicui18n.so.75")
  library(Seurat)
  library(ggplot2)
  library(dplyr)
  ...
  cluster_number <- length(levels(Idents(sobj)))
    ...
    png_file <- file.path(
    temp_png_folder,
    sprintf("Resolution:%.2f_ClusterNumber:%d.png", resolution, cluster_number)
  )
  ...
  message("All resolutions processed! Creating final PDF...")
  resolution_summary <- dplyr::bind_rows(
  ...
  all_png_files <- setNames(
    ...
    vapply(results, function(x) as.character(x$resolution), "")
  )
  final_pdf <- file.path(output_folder, "All_Resolutions_Combined.pdf")
  pdf(final_pdf, width = 18, height = 18)
  n_rows <- nrow(resolution_summary)
